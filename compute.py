...
def compute_noise(params, user_bandwidth):
    """计算用户噪声功率，使用用户实际分配的带宽
    Args:
        params: 系统参数字典
        user_bandwidth: 用户实际分配的带宽 (Hz)
    Returns:
        噪声功率 (W)
    """
    ...
    N0 = boltzmann_constant * Teq * user_bandwidth
    return N0
...
def main():
    ...
    for i, user_pos in enumerate(user_positions):
        ...
        # 使用用户实际分配的带宽计算噪声
        noise = compute_noise(params, user_bandwidth)
        ...
        # 频谱效率
        spectral_efficiency = np.log2(1 + snr) if snr > 0 else 0
        # 有效带宽修正
        beff = user_bandwidth
        # 实际吞吐量（bps）
